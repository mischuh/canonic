...
human reviews the proposed context change. Canon **owns no write-path** (PRD non-goal) — it only
...
_BRANCH_PREFIX = "canon/ingest-"


class PullRequestPublisher(Protocol):
    """The git/gh seam (SPEC-E4 §6) — injected so the auto-PR step is testable and Canon-agnostic.
...
class SubprocessPublisher:
    ...
    :class:`CanonError` on a non-zero exit so a failed git/gh step never passes silently. Canon
    ...
    async def _run(self, *args: str) -> str:
        ...
        if proc.returncode != 0:
            ...
            raise CanonError(f"auto-PR step failed: {' '.join(args)}: {detail}")
...
class AutoPRPublisher:
    ...
    def _branch_name(emission: EmissionResult) -> str:
        """Deterministic branch name: ``canon/ingest-<hash>`` over the emission's JSON (§7)."""
    ...
    def _title(emission: EmissionResult) -> str:
        ...
        return f"chore(canon): ingest reconciliation — {n} diff{'s' if n != 1 else ''}"
