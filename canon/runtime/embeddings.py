...
``canon[embeddings]`` add-on, **not** bundled by default. When the add-on is absent (or the
...
The runtime satisfies E6's :class:`~canon.knowledge.embeddings.Embedder` seam: ``embed``
...
:class:`~canon.airgap.EgressPolicy` check is needed on the default path. A future hosted
...
class EmbeddingRuntime:
    ...
    Constructed from an :class:`~canon.config.EmbeddingConfig`. The backend is loaded
