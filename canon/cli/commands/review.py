"""``canon review`` — interactive, item-by-item proposal review (GH-150, E7 §3).
...
from canon.config import LOCAL_STATE_DIR, find_project_root
...
def review(
    ...
    if run_dir is None:
        if run_id is not None:
            ...
            _console.print("[yellow]no pending-diff runs found — run `canon ingest` first[/yellow]")
        raise typer.Exit(1)
