...
harness. Each entry is a friendly ``name`` plus the same fields as a ``canon.yaml`` ``llm`` block,
parsed into an :class:`~canon.config.LLMConfig` so the harness drives the exact production path.
"""
    ...
    Expected shape::
        ...
        candidates:
          - name: small-local
            ...
            model: qwen2.5:3b
            api_key_ref: env:CANON_LLM_KEY   # optional; local servers need none
