...
``.canon/`` (local, rebuilt from committed Markdown, never committed) or in RAM for tests.

No scope/tag/usage_mode filtering happens here: the index returns ranked candidates and
:class:`~canon.knowledge.retrieval.KnowledgeSearch` post-filters the (small) corpus,
...
class KnowledgeIndex:
    ...
    def build(cls, pages: Iterable[KnowledgePage], *, path: Path | None = None) -> KnowledgeIndex:
        ...
        that directory under ``.canon/`` (production). One document per page; ``tags`` is
