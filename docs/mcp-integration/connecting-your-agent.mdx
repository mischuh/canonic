...
The HTTP daemon also serves unauthenticated `/livez` and `/readyz` probe endpoints for load balancers and Kubernetes. See [Health endpoints](/cli-reference/mcp#health-endpoints).
...
For a runnable end-to-end example with a real IdP, see [Marketplace with Keycloak](/guides/marketplace-keycloak): a local Keycloak plus a dockerized daemon, with role and tenant claims flowing from the login into the query.

**Headless/agentic clients**: `mode: proxy` also accepts an IdP-signed identity assertion (SEP-990 ID-JAG) in place of the interactive Authorization Code + PKCE flow, for a client that already holds an established identity from the IdP and cannot open a browser to log in:
  ...
  auth:
    oauth:
      ...
      identity_assertion:
        ...
        audience: canonic-mcp   # optional, defaults to this daemon's own issuer identifier
```

Under a [tenancy or role policy](/concepts/tenancy-and-access-control#principal-binding), the IdP must sign the tenancy and role claims into the assertion itself, since canonic reads the principal from the verified assertion's claims (after `claim_mapping`) and from nothing else. The answer is scoped to the employee named in the assertion, not to the agent, and `.canonic/events.jsonl` records both: `"user": {"subject": "<employee>", "acted_via": "<agent client_id>"}`.

Prefer this over a static `mcp.auth.tokens` entry for an agentic client that already participates in your IdP: identity stays centrally revocable and per-caller, instead of a shared secret to distribute and rotate by hand. Static tokens remain fully supported for pipelines or clients with no IdP identity of their own, and the two compose on the same daemon, same as `tokens` and `oauth` do generally. Signature and replay verification of the assertion itself is FastMCP's, not canonic's, to reimplement.

**CI pipelines and scheduled jobs**: prefer the OAuth 2.0 client-credentials grant against your IdP over a static `mcp.auth.tokens` entry. The job requests a short-lived token from the IdP before calling canonic (FastMCP's client-side `ClientCredentialsOAuthProvider` implements this), so the credential is revocable at the IdP and never written to `canonic.yaml`. The daemon verifies the resulting token like any other bearer token, so no server-side setting changes. Static tokens remain supported.
...
<Warning>
Upgrading a `proxy`-mode deployment from a canonic release built on FastMCP 3 to one built on FastMCP 4 is a one-time re-authorization event, if and only if your `issuer_url` differs from the daemon's public base URL. FastMCP 4 stamps `issuer_url` into the `iss` claim of the tokens it mints, so tokens minted before the upgrade no longer validate and every client, including its refresh token, has to log in again. Deployments where the two URLs are equal are unaffected. Announce the window before restarting, and confirm afterwards with `canonic mcp status`, which reports the FastMCP version the running daemon serves.
</Warning>
...
`query`, `run_sql` and `run_report` can run as background tasks on `--transport http`, so a client that supports the MCP tasks extension can poll a slow query instead of holding the connection open for its full duration:
...
mcp:
  tasks:
    enabled: true
    backend: memory   # memory | redis, redis needed only to coordinate multiple daemon processes
```

This is additive, not a replacement: a client that doesn't declare the tasks capability still gets the same synchronous response as before, on the same tool. Requires the `canonic[tasks]` extra (`pip install "canonic[tasks]"`). The daemon refuses to start with a clear error if `mcp.tasks.enabled: true` is set and the extra isn't installed. Not available under `--transport stdio`, a local subprocess has no reason to hold a request open in the first place. See [config schema](/reference/config-schema#mcp) for the full field list.

### Listing cache hints

A conforming client may cache `list_metrics`/`describe_metric`/tool listings for `mcp.cache_ttl_seconds` (default 300) before re-fetching, cutting round trips for a client that re-lists frequently:

```yaml
# canonic.yaml
mcp:
  cache_ttl_seconds: 0   # disables the hint entirely, the default is 300
```

The hint is scoped `private` always, never shared across callers, since a shared cache would leak one caller's listing to another once tenancy or role-scoped tools are in play. Tool call results (`query`, `run_sql`, `run_report` answers) are untouched, only listings. Honoring the hint is opt-in and modern-era-only on the client side, so a legacy client or one that doesn't ask for caching is unaffected either way.
