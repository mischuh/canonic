...
description: "The broadest example: every metric binding kind, all four guardrail kinds, finality, and assertions."
---
...
This is the broadest example in the set: it exercises every [metric binding kind](/concepts/contracts-and-guardrails#beyond-single-composable-metrics) canonic supports, all four guardrail kinds, finality/restrict-source, query-based assertions, and a global knowledge base, in one project.
...
canonic status
# Canonic project: saas-analytics-demo (version 1)
```
...
```json
// refunds + trials silently excluded, and provisional rows excluded under board_reporting
