...
description: "A multi-merchant marketplace on SQLite demonstrating tenant isolation, role-based authorization, and column masking."
---
...
<Info>Full source: [`examples/marketplace/`](https://github.com/mischuh/canonic/tree/main/examples/marketplace). To run it behind a real identity provider, see [Marketplace with Keycloak](/guides/marketplace-keycloak).</Info>
...
`canonic.yaml` resolves its five MCP tokens from environment variables (`token_ref: env:...`), so `canonic mcp start --transport http` needs them set first. Any value works for local development:

```bash
export CANONIC_MCP_TOKEN_BYTE_GADGETS_VIEWER=byte-gadgets-viewer-local-dev
export CANONIC_MCP_TOKEN_BYTE_GADGETS_ADMIN=byte-gadgets-admin-local-dev
export CANONIC_MCP_TOKEN_URBAN_THREADS_VIEWER=urban-threads-viewer-local-dev
export CANONIC_MCP_TOKEN_URBAN_THREADS_ADMIN=urban-threads-admin-local-dev
export CANONIC_MCP_TOKEN_PLATFORM_OPS=platform-ops-local-dev
```
...
This happens because `contracts/policies/tenancy.yaml` sets `on_missing_principal: deny`. A CLI-direct query with `--tenant` binds no role claim, so it runs as the `default_role` of `roles.yaml` (`merchant_viewer`).
...
| --- | --- | --- |
| `byte-gadgets-viewer` | `merchant_id: byte-gadgets`, `roles: [merchant_viewer]` | `revenue`/`order_count`/`aov` only, scoped to Byte Gadgets, `customer_email`/`customer_phone` denied as dimensions |
...
The five identities above are static bearer tokens, the simplest setup. The same identities can also come from a real identity provider through `mcp.auth.oauth`. The [Keycloak guide](/guides/marketplace-keycloak) runs this exact project behind a local Keycloak with a browser login, and both mechanisms can be active on one daemon.
...
merchant_viewer (byte-gadgets-viewer's role), customer_email dimension:
  error unreachable: dimension 'customer_email' is not declared on any reachable source
...
`merchant_viewer`'s `dimensions.deny: [customer_email, customer_phone]` rejects the query outright, with the same error a nonexistent dimension produces. `merchant_admin` re-opens the dimension (`deny: []`) and its `masking: [{ column: customers.customer_email, strategy: partial }]` redacts the value instead.
...
The Keycloak variant lives outside the example in [`scripts/local_idp/`](https://github.com/mischuh/canonic/tree/main/scripts/local_idp) (compose file, realm export, and a `canonic.docker.yaml` overlay), so the example itself stays unchanged.
