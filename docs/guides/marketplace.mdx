...
| `byte-gadgets-admin` | `merchant_id: byte-gadgets`, `roles: [merchant_admin]` | Same metrics, scoped to Byte Gadgets, `customer_email` is **masked** (`partial`), `run_sql: true` but still refused (see "run_sql's two gates"), may save/compose their own queries and reports (`manage_saved_content: true`) |
...
| `platform-ops` | `roles: [platform_analyst]` (no `merchant_id`) | `tenancy_exempt: true`, every metric (`allow: ["*"]`), every knowledge tag, `run_sql: true` and actually allowed, `manage_saved_content: true` |

`merchant_admin` `inherits: merchant_viewer` (field-level override, not a list-merge): it re-opens `dimensions.deny` to `[]`, adds a `masking` rule, and grants `manage_saved_content` instead. `merchant_viewer` leaves `manage_saved_content` at its default `false` — a plain viewer may run ad hoc queries but not accumulate their own saved queries/personal reports (see [The `manage_saved_content` gate](/concepts/tenancy-and-access-control#the-manage_saved_content-gate)); `save_query`/`compose_report`/etc. as a `byte-gadgets-viewer` token fails with `tenant_forbidden` the same way `run_sql` does for that role.
