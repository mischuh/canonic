...
| `context` | `str \| null` | `null` | Required (non-empty) when `kind: restrict_source` or `kind: min_trust`, the named context the restriction applies in (e.g. `board_reporting`). |
