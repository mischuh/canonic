  ...
  auth:
    tokens:
      ...
      - client_id: merchant-4711-agent
        ...
        claims: { merchant_id: '4711', roles: [merchant_viewer] }   # SPEC-E12 §7, tenancy/RBAC only
    oauth:
      ...
      claim_mapping:                                        # SPEC-E12 §7, tenancy/RBAC only
...
`contracts/policies/` (`tenancy.yaml` / `roles.yaml`) is optional — its presence is the feature switch for [tenancy & access control](/concepts/tenancy-and-access-control); a project without it behaves exactly as it did before that layer existed.
