...
| `logging` | `LoggingConfig` | `level: WARNING`, `format: text` | Log level, optional file, text/json format, and size-based rotation, see [below](#logging). |
| `instrumentation` | `InstrumentationConfig` | see [below](#instrumentation) | Size and retention limits for the local event log `.canonic/events.jsonl`. |
...
logging:
  level: WARNING
  file: null                # null logs to stderr
  format: text              # text | json
  max_bytes: 52428800       # 50 MB, 0 disables rotation
  backup_count: 5
```

| Field | Type | Default | Governs |
| --- | --- | --- | --- |
| `level` | `str` | `WARNING` | Log level of the `canonic` logger. Override with `CANONIC_LOG_LEVEL`. |
| `file` | `str \| null` | `null` | Log file path. Without it, records go to stderr, never stdout, because stdio MCP carries JSON-RPC there. Override with `CANONIC_LOG_FILE`. |
| `format` | `text \| json` | `text` | One text line or one JSON object per record. Override with `CANONIC_LOG_FORMAT`. |
| `max_bytes` | `int` | `52428800` | Rotates `file` at this size. Also rotates the MCP daemon's `.canonic/mcp.log` each time the daemon starts. `0` disables rotation. |
| `backup_count` | `int` | `5` | Number of rotated files kept (`canonic.log.1`, `canonic.log.2`, and so on). The oldest one is deleted. |

The daemon writes `.canonic/mcp.log` through an inherited file descriptor, so it can only rotate on daemon start, not while the daemon runs. Set `logging.file` if you need rotation during a long-running daemon.
...
instrumentation:
  events:
    max_bytes: 52428800     # 50 MB, 0 disables rotation
    retention_days: null    # null keeps segments forever
    max_segments: null      # null keeps all segments
    compress: true
```

| Field | Type | Default | Governs |
| --- | --- | --- | --- |
| `events.max_bytes` | `int` | `52428800` | Size at which the active `.canonic/events.jsonl` rotates. `0` disables rotation. |
| `events.retention_days` | `int \| null` | `null` | Deletes segments whose rotation time is older than this many days. |
| `events.max_segments` | `int \| null` | `null` | Keeps only this many of the newest segments. |
| `events.compress` | `bool` | `true` | Gzips segments (`events-<UTC>.jsonl.gz`). |

Rotation moves `served_answer` events into a segment. Funnel, outcome, and reconcile events stay in the active file, so limits never affect onboarding state or feedback history. Retention only deletes whole segments and is evaluated when a rotation happens. See [The event log](/concepts/instrumentation-and-eval#growth-and-retention).
  ...
  auth:
    ...
    oauth:
      ...
      claim_mapping:                                        # tenancy/RBAC only
        ...
        roles: roles
      identity_assertion:                                   # proxy mode only, headless/agentic clients
        trusted_issuers: [https://login.acme-corp.com]
        audience: canonic-mcp
  tasks:                                                    # http transport only
    enabled: false
    backend: memory   # memory | redis
    url: null          # required (and only used) when backend is redis
  cache_ttl_seconds: 300   # 0 disables listing cache hints entirely
```
...
| `auth.oauth.identity_assertion` | `McpIdentityAssertionConfig \| null` | `null` | `proxy` mode only. Accepts an IdP-signed identity assertion (SEP-990 ID-JAG) in place of the interactive Authorization Code + PKCE flow, for headless/agentic clients that already hold an established identity from the IdP. Rejected in `jwt` mode. |
| `auth.oauth.identity_assertion.trusted_issuers` | `list[str]` | n/a (required, non-empty) | Issuer (`iss`) values the proxy accepts on an assertion. Each entry must start with `http://` or `https://`. |
| `auth.oauth.identity_assertion.audience` | `str \| null` | `null` | Expected `aud` value on the assertion. Defaults to this daemon's own issuer identifier when omitted. |
| `tasks.enabled` | `bool` | `false` | Runs `query`, `run_sql` and `run_report` as optionally-async tools on `http` transport, so a client that declares the `io.modelcontextprotocol/tasks` capability can poll a long-running query instead of holding the request open. A client that doesn't opt in still gets a synchronous result. Requires the `canonic[tasks]` extra installed. The daemon refuses to start otherwise. Never applies to `stdio`. |
| `tasks.backend` | `"memory" \| "redis"` | `"memory"` | The Docket backend running task state. `memory` is single-process, good enough for one daemon instance. `redis` coordinates tasks across multiple daemon processes/workers. |
| `tasks.url` | `str \| null` | `null` | Docket backend URL (e.g. `redis://localhost:6379/0`). Required when `backend` is `redis`, rejected when `backend` is `memory`. |
| `cache_ttl_seconds` | `int` | `300` | How long (in seconds) a client may treat a listing (`tools/list`, `resources/list`, `prompts/list`, `server/discover`) as fresh, a SEP-2549 client-side cache hint applied uniformly across all four. `0` disables the hint entirely, byte-identical to a server that never set one. Must be `>= 0`. The scope is always `private`, never shared across callers, and tool call results are unaffected, only listings. |

This block only matters for `canonic mcp start --transport http`. The default `stdio` transport is a local subprocess and needs no auth, process-level trust is enough. `http` transport is network-reachable, though, so it refuses to start unless at least one mechanism resolves: a token here (or via the `--token-ref` CLI override) and/or `auth.oauth`. When both are configured, a request is checked against the static token map first (no network call), falling through to OAuth verification if no static token matches: the two stay independently revocable, a token entry by editing `canonic.yaml`, an OAuth-issued token at the IdP. `canonic mcp status` reports which mechanism(s) are active on a running daemon. See [Connecting your agent](/mcp-integration/connecting-your-agent#remote-enterprise-deployment) for the full remote-deployment walkthrough, and [Marketplace with Keycloak](/guides/marketplace-keycloak) for a working `proxy`-mode example against a real IdP.
