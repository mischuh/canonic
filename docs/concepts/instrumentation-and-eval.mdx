...
`user` is the verified caller on the `http` transport and `null` on stdio and the CLI. It's a plain string (the token's `client_id`) for static tokens and OAuth logins. For a call made with an [identity assertion](/mcp-integration/connecting-your-agent), where an agent acts for an employee, it's an object instead: `{"subject": "<employee>", "acted_via": "<agent client_id>"}`. `subject` is the employee whose tenant and roles scoped the answer, and `acted_via` is the agent that made the call.
...
The log is local by design, so in a large deployment it needs a bound. By default the active `.canonic/events.jsonl` rotates once it reaches 50 MB:

- `served_answer` events, the only high-volume kind, move into an immutable segment `events-<UTC>.jsonl.gz` next to it.
- Funnel, outcome, and reconcile events stay in the active file. They are small, and the onboarding funnel, [feedback loop](/concepts/ingestion-and-reconciliation#feedback-loop-e11), and audit depend on them.
- `canonic status` and `canonic audit` read segments and the active file transparently. Nothing needs to be merged by hand.
- Optional `retention_days` and `max_segments` delete whole segments, oldest first. Nothing is summarized, an expired segment is simply gone. Both default to unlimited.
- Concurrent writers, for example the MCP daemon and a CLI command, are serialized with a file lock.

Limits are set under `instrumentation.events` in `canonic.yaml`, see the [config reference](/reference/config-schema#instrumentation). This works the same in [air-gapped](/concepts/llm-runtime#air-gapped-mode--enforced-not-advisory) mode, where the log is written locally and never sent anywhere.
