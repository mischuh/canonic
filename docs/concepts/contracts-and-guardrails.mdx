...
`severity: error` (the default) blocks the query; `severity: warn` lets it through with a `warnings[]` entry instead. `mandatory_filter` has no "block" outcome to gate this way — its predicate is always injected regardless of severity, and `severity: warn` there only adds the `warnings[]` entry on top.
...
`canonic report` includes a trust-tier worklist ranking every canonical metric worst-tier-first; see [`canonic report`](/cli-reference/status-report#canonic-report).
