...
| `manage_saved_content` | `bool` | `false` | Whether this role may call any of the six self-service saved-query/personal-report capabilities (`save_query`, `list_saved_queries`, `delete_query`, `compose_report`, `update_report`, `delete_report`), see [The `manage_saved_content` gate](#the-manage_saved_content-gate). |
...
## The `manage_saved_content` gate

Not every caller who can read canonic's data should be able to manage their own saved queries and personal reports — `save_query`, `list_saved_queries`, `delete_query`, `compose_report`, `update_report`, and `delete_report` (see [Saved queries and personal reports](/reference/report-schema#saved-queries-and-personal-reports)) are gated as one group, fail-closed, the same shape as the `run_sql` gate: a role with `manage_saved_content: false` (the default) is refused outright with `TENANT_FORBIDDEN`, checked before any other validation (missing identity, unknown id) runs.

```yaml
roles:
  viewer:
    metrics: { allow: ["revenue", "order_count"] }
    manage_saved_content: false   # the default — spelled out here for clarity
  analyst:
    inherits: viewer
    manage_saved_content: true    # may save/list/delete their own queries and reports
```

With no role policy loaded at all, this capability is unrestricted, matching every other gate's "absence is the feature switch" rule. Once a policy is loaded, an unauthenticated caller (no principal, or a CLI `--tenant` override, which never carries roles) resolves to the anonymous principal — denied, unless `default_role` grants it.
