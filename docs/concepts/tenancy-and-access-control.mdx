...
`dimensions.deny` (and `dimensions.allow`) is enforced at query time and in discovery. A query that groups by a denied dimension, or filters on the column behind one, fails with the same `UNREACHABLE` error an undeclared name produces, so the error does not reveal that the name exists. Denied dimensions are also left out of `describe_metric`, `get_overview` and the related-dimension suggestions, and metrics outside a role's allow list are left out of the related-metric suggestions. A join-qualified name such as `pickup.city` is matched against a bare `deny: [city]`. `masking` (below) is the complementary tool: it lets a column be queried but rewrites its *value*.
