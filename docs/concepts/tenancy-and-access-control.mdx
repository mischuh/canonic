...
`dimensions.deny` (and `dimensions.allow`) is enforced at query time and in discovery. A query that groups by a denied dimension fails with the same `UNREACHABLE` error an undeclared dimension produces, so the error does not reveal that the name exists. Denied dimensions are also left out of `describe_metric`, `get_overview` and the related-dimension suggestions. A join-qualified name such as `pickup.city` is matched against a bare `deny: [city]`. `masking` (below) is the complementary tool: it lets a column be queried but rewrites its *value*.

<Warning>
`dimensions.deny` gates the `dimensions` of a query, not `filters`. A filter expression that references a denied column is not yet rejected.
</Warning>
...
**MCP identity assertion** covers an agent acting for a specific employee with no browser session, such as a scheduled report or an internal tool. The corporate IdP signs an assertion (SEP-990 ID-JAG) naming the employee, the agent exchanges it for a short-lived token, and the tenant and roles are read from the claims the IdP signed into that assertion, with the same `claim` names and `claim_mapping` as above. Put the tenancy and role claims into the assertion itself: they are the only claims that count on this path. The resulting principal is indistinguishable to the compiler from the one the same employee gets through an interactive login, and an assertion without the tenancy claim fails with `TENANT_UNRESOLVED` like any other token. This path exists only on the `http` transport, since the CLI never exchanges anything with an IdP. See [identity assertion setup](/mcp-integration/connecting-your-agent).
