...
<Tabs>
  ...
  <Tab title="github_copilot">
    Routed through your GitHub Copilot subscription. No `base_url`, and **no `api_key_ref`**: authentication is a one-time device-code flow (a browser prompt on first use), and the resulting credential is cached on disk and reused after that:
...
All four are reached through [litellm](https://github.com/BerriAI/litellm) behind one interface, with no per-provider branching anywhere else in canonic. `tasks:` optionally overrides the model per task (`draft`, `reconcile`):
