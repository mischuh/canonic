...
| [`validate`](/cli-reference/status-report) | Validate every contract against the project's semantic sources (read-only, no connection). |
| [`audit`](/cli-reference/status-report) | Show event-log figures: counts, errors, latency, freshness. |
