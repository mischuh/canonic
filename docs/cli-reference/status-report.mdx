...
### `canonic report --bundle`

```bash
canonic report --bundle diagnostics.json
```

Writes a single JSON file for attaching to a bug report: canonic/Python/platform version, the serving contract schema, the redacted `canonic.yaml` (connection secrets are already stored as references — `env:`/`keyring:`/`file:` — never literals; free-form connector `params` entries are additionally redacted defensively), and the onboarding funnel plus event-log summary. Contains no query results. Nothing is sent anywhere — review the file yourself before attaching it.
