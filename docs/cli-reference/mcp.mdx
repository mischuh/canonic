...
With `--transport stdio` (the default), the daemon runs in the foreground over stdio; the MCP client manages its process lifetime — no auth layer, since a local subprocess is inherently process-trusted. If a [tenancy policy](/concepts/tenancy-and-access-control) is active, a `stdio` session has no per-request auth to derive a principal from, so `--tenant <id>` must be passed explicitly.
...
Shows the PID, transport, address and auth state (for HTTP — which mechanism(s) are active, e.g. `token, oauth-proxy`), version, and start time. Warns if the running daemon's version differs from the current CLI version; in which case, restart it with `canonic mcp stop && canonic mcp start`.
