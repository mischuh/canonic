...
- **Drift review flag.** ``meta.bound_fingerprints`` records the measure-definition
  ...
  ``{{ sl:….expr }}`` auto-updates (:mod:`canonic.knowledge.rendering`), but the surrounding
...
class DriftDetector:
    ...
    def flagged_for_review(self, page: KnowledgePage, entity_index: EntityIndex) -> list[str]:
        ...
        :meth:`~canonic.knowledge.validation.EntityIndex.current_fingerprint`. A name whose
