...
- **Visibility** — a user's search sees ``global`` pages + their *own* ``user/<id>`` pages,
  ...
  (:func:`~canonic.knowledge.loader.user_from_path`), **not** row-level security; a real trust
...
class ScopeResolver:
    ...
    def visible_pages(
        ...
        page corpus rather than a :class:`~canonic.knowledge.validation.PageIndex` because
        filtering requires each page's owner (parsed from its path via
        :func:`~canonic.knowledge.loader.user_from_path`).
        """
        return [
            page
            for page in pages
