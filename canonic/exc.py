"""Canonic-wide exceptions and the canonical error registry (SPEC E7+E8 §6.1)."""
...
class CanonicError(Exception):
    """Base exception for all canonic errors.
...
class ConnectionError(CanonicError):  # noqa: A001 — intentionally shadows builtin in this namespace
...
class StructuredOutputError(CanonicError):
...
class StructuredOutputUnsupported(CanonicError):
...
class RetriesExhausted(CanonicError):
...
class AirGappedViolation(CanonicError):
...
class CredentialError(CanonicError):
...
class EvalDatasetError(CanonicError):
    ...
    Operator-input error for the ``canonic eval baseline`` harness: a labeled ``draft`` case that is
