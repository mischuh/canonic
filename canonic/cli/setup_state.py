"""Checkpoint state for the ``canonic setup`` wizard (SPEC E1 §4 resumability).

Progress is persisted to ``.canonic/setup-state.json`` after each completed step so
...
from canonic.config import LOCAL_STATE_DIR, Connection, LLMConfig
...
def save_state(root: Path, state: SetupState) -> None:
    """Persist state to ``.canonic/setup-state.json``, creating ``.canonic/`` if needed."""
