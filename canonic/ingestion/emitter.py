...
snapshot under ``raw-sources/`` and the local, git-ignored event log under ``.canonic/``) is
...
from canonic.connectors.base import AcquisitionTier  # noqa: TC001 — Pydantic resolves at runtime
...
from canonic.instrumentation.models import ReconcileDecisionEvent
from canonic.semantic.models import Provenance  # noqa: TC001 — Pydantic resolves at runtime
...
class DiskEventLog:
    """Writes reconcile-decision events into the shared ``.canonic/events.jsonl`` substrate (SPEC-E16 §11 S4).

    Each entry is mapped to a :class:`~canonic.instrumentation.models.ReconcileDecisionEvent`
    and appended through :class:`~canonic.instrumentation.events.DiskAnswerEventLog` so both
