"""Pending-diff persistence — bridge between canonic ingest and a later review/apply (SPEC-E4 §6, E1 §7).

After each ingest run the pipeline writes a timestamped directory under ``.canonic/pending-diffs/``
so the emitted proposals survive process exit and a subsequent ``canonic review`` / ``canonic apply``
can act on them without re-running the reconciliation engine.

Layout written per run::

    .canonic/pending-diffs/<run-id>/
...
if TYPE_CHECKING:
    from canonic.ingestion.emitter import EmissionResult, EmittedDiff
...
class PendingDiffStore:
    """Persists emitted diffs to ``.canonic/pending-diffs/<run-id>/`` (SPEC-E4 §6, GH-149).
...
def latest_run_id(project_root: Path) -> str | None:
    """Return the name of the most-recent run directory under ``.canonic/pending-diffs/``.
...
class PendingRun:
    ...
    produced by re-running :class:`~canonic.ingestion.emitter.DiffEmitter` over the persisted
